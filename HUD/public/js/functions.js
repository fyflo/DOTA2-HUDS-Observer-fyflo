...
function loadTeam(team) {
  ...
  $("#flags")
    .val(team ? team.country_code : "default")
    .formSelect();
  $("#delete_team")
  ...
  $("#logo_img")
    .attr("src", team && team.logo ? "/storage/" + team.logo : "")
    .hide();
...
function listPlayers(defaultPlayer) {
  loadPlayers(function (players) {
    ...
    players.forEach(function (player, id) {
      ...
      if (defaultPlayer && defaultPlayer == player._id)
    ...
    huds.forEach(function (hud) {
      ...
      if (hudList[hud]) {
        hudList[hud].forEach(function (inst) {
          ...
          $temp
            .appendTo($("<tr></tr>"))
            .attr("data-hid", inst._id)
            .appendTo($hudsTable);
          $temp.find("#delay").val(inst.delay);
          $temp
            .find("td:eq(3)")
            .html(
              "<a href='/huds/" + inst._id + "'>/huds/" + inst._id + "</a>"
            );
          $temp.find("#warnings").html('<i class="material-icons">done</i>');

          if (inst.enabled == true)
            $temp
              .find("input[type='checkbox'][id$='huds']")
              .prop("checked", "true");
        }, this);
      }
      let $status = $hudRow.find("#warnings i");
      if (
        !files[hud].includes("template.pug") ||
        !files[hud].includes("index.js") ||
        !files[hud].includes("style.css")
      ) {
        let tip =
          "Missing files:" +
          (!files[hud].includes("template.pug") ? " template.pug," : "") +
          (!files[hud].includes("index.js") ? " index.js," : "") +
          (!files[hud].includes("style.css") ? " style.css," : "");
        $status
          .addClass("tooltipped")
          .attr({
            "data-position": "top",
  ...
  $.ajax({
    ...
    error: function () {
      $("tr[data-hid='" + data.id + "']")
        .find("#warnings")
        .html(
          '<i class="material-icons tooltipped" data-position="top" data-tooltip="Error during request">error_outline</i>'
        );
    },
    success: function () {
      $("tr[data-hid='" + data.id + "']")
...
function loadPlayer(player) {
  ...
  $("#delete_player")
    .removeClass("disabled")
    .addClass(!player ? "disabled" : "");
  $("#id").val(player ? player._id : "");
  $("#avatar_img")
    .attr("src", player && player.avatar ? "/storage/" + player.avatar : "")
    .hide();
